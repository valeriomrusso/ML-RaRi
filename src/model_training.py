...
def train_model_nn_ranged(fold_no, build_model, x_train, y_train, x_val, y_val):
...
def train_model_ridge_ranged(fold_no, build_model, x_train, y_train, x_val, y_val):
    """Training con gestione errori"""
    try:
        temp_dir = tempfile.mkdtemp()
        tuner = kt.RandomSearch(
...
def train_model_ridge_fixed(model, batch_size, X_train, X_val, Y_train, Y_val):
    final_history = model.fit(
        X_train, Y_train,
        batch_size=batch_size,
        epochs=200,  # Puoi regolare il numero di epoche

...
def build_model_nn_fixed(units, dropout, num_layers, units_hidden, learning_rate, momentum, reg, task):
    ...
    # Compilazione
    model.compile(
        optimizer=keras.optimizers.SGD(learning_rate, momentum), loss='mse', metrics=metrics)
...
def build_model_ridge_ranged_tuner(task):
    def build_model(hp):
        ...
        model.compile(loss='mse', optimizer = keras.optimizers.SGD(hp.Float("lr", min_value=1e-6, max_value=1e-1, sampling="log")), metrics=metrics)
        return model
    return build_model
...
def build_model_ridge_fixed(reg, learning_rate, momentum, task):
    ...
    model.add(keras.layers.Input(input_shape))
    model.add(keras.layers.Dense(output_shape, kernel_regularizer= keras.regularizers.l2(reg), activation= actfun))
    model.compile(loss='mse', optimizer = keras.optimizers.SGD(learning_rate, momentum), metrics=metrics)
    return model